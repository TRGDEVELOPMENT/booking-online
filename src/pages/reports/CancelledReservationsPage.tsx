...
import { Ban, Download, FileText, Filter } from 'lucide-react';
...
export default function CancelledReservationsPage() {
    ...
    const fetchCancelledReservations = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from('reservations')
          .select('id, document_number, customer_name, model, submodel, net_price, created_at, updated_at')
          .eq('status', 'cancelled')
  ...
  return (
    <>
      <Header 
        title="รายงานการยกเลิกการจอง" 
        subtitle="รายงานสรุปใบจองที่ถูกยกเลิก"
      />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-card rounded-lg border border-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">จำนวนใบจองที่ยกเลิก</p>
          ...
          <div className="p-4 bg-card rounded-lg border border-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">มูลค่าที่สูญเสีย</p>
        ...
        <div className="bg-card rounded-lg border border-border overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p>ไม่พบใบจองที่ยกเลิกในเดือนที่เลือก</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {reservations.map((reservation, index) => (
                  <TableRow key={reservation.id}>
                    ...
                    <TableCell>
                      {new Date(reservation.updated_at).toLocaleDateString('th-TH', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                      })}
