...
const stageConfig = [
  { stage: 'cashier', label: 'ตรวจสอบการชำระเงิน (แคชเชียร์)', role: 'cashier' },
  { stage: 'review', label: 'ตรวจสอบใบจอง (หัวหน้าทีมขาย)', role: 'sale_supervisor' },
  { stage: 'approval', label: 'อนุมัติใบจอง (ผู้จัดการฝ่ายขาย)', role: 'sale_manager' },
];
...
export default function SettingsApprovalChainPage() {
  ...
  // Fetch templates when team changes
  useEffect(() => {
    if (!selectedTeamId) {
      setTemplates({});
      setSavedTemplates({});
    ...
    const fetchTemplates = async () => {
      const { data } = await supabase
        .from('team_approval_templates' as any)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('บันทึกเทมเพลตสายอนุมัติสำเร็จ');
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <>
      <Header
        ...
        subtitle="ตั้งค่าเทมเพลตผู้รับผิดชอบสายอนุมัติ สำหรับใบจองใหม่ของแต่ละทีมขาย"
      />
            ...
            <>
              ...
              {!selectedTeamId && (
                <div className="text-center py-12 text-muted-foreground">
                  กรุณาเลือกทีมขายเพื่อตั้งค่าเทมเพลตสายอนุมัติ
                </div>
              )}
            </>
          )}
